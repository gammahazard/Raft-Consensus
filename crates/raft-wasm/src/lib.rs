...
impl Guest for RaftNodeComponent {
    ...
    fn on_message(from_node: u64, msg: WitRaftMessage) {
        NODE.with(|n| {
            ...
            if let Some(ref mut _node) = *node_ref {
                let _internal_msg = from_wit_message(msg);
                let _ = from_node; // Suppress unused warning for now
                // TODO: Handle message and generate responses
            }
        });
    }
